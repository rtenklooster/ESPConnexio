...
#define PLUGIN_NAME_012   "OregonV2"
...
boolean Plugin_012(byte function, struct NodoEventStruct *event, char *string)
  ...
  case PLUGIN_MMI_IN:
    ...
    if(GetArgv(string,TempStr,1))
      {
      if(strcasecmp(TempStr,PLUGIN_NAME_012)==0)
        {
        if(event->Par1>0 && event->Par1<255 && event->Par2>0 && event->Par2<=USER_VARIABLES_MAX)
          {
          event->Command = PLUGIN_ID_012; // Plugin nummer  
  ...
  case PLUGIN_MMI_OUT:
    ...
    sprintf(string,"%s",PLUGIN_NAME_012);
