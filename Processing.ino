  ...
  if(PreviousProcessing!=Processing)
    ...
    if(Processing)
      ...
      if(RequestForConfirm)                                                     // Een event kan een verzoek bevatten om bevestiging. Doe dit dan pas na alle verwerking. 
        ...
        TempEvent.Type                  = ESP_TYPE_SYSTEM;                     // Event is niet voor de gebruiker bedoeld
...
byte ProcessEvent(struct NodoEventStruct *Event)
  ...
  if(Settings.Lock && (Event->Port==VALUE_SOURCE_RF || Event->Port==VALUE_SOURCE_IR ))
    {
    Continue=false;
    if(Event->Type==ESP_TYPE_COMMAND)
      {
      if(Event->Command==CMD_LOCK                   )Continue=true;
      }
    else if(Event->Type==ESP_TYPE_EVENT)
      {
    ...
    else if(Event->Type==ESP_TYPE_SYSTEM)
      {
...
boolean CheckEvent(struct NodoEventStruct *Event, struct NodoEventStruct *MacroEvent)
  ...
  if(MacroEvent->Command == EVENT_WILDCARD)                                                                                 // is regel uit de eventlist een WildCard?
    if( MacroEvent->Par1==VALUE_ALL          ||   MacroEvent->Par1==Event->Port)                                            // Correspondeert de poort of mogen alle poorten?
      if((MacroEvent->Par2&0xff)==VALUE_ALL  ||  (MacroEvent->Par2&0xff)==Event->Command && (Event->Type==ESP_TYPE_EVENT || Event->Type==ESP_TYPE_RAWSIGNAL))   // Correspondeert het commando deel
  ...
  // ### TIME:
  if(Event->Type==ESP_TYPE_EVENT && Event->Command==EVENT_TIME)                // het binnengekomen event is een clock event.
    {
...
boolean QueueAdd(struct NodoEventStruct *Event)
  ...
  if(QueuePosition<EVENT_QUEUE_MAX)
    ...
    if(Event->Flags & TRANSMISSION_VIEW)                                        // Als alleen weergeven, dan direct doen en niet in de queue plaatsen.
      ...
      {
      if(Queue[QueuePosition-3].Type==ESP_TYPE_SYSTEM && Queue[QueuePosition-3].Command==SYSTEM_COMMAND_QUEUE_EVENTLIST_SHOW)
        {
...
void QueueProcess(void)
  ...
  if(QueuePosition)
    {
    if(Queue[0].Type==ESP_TYPE_SYSTEM)
      {
...
byte QueueSend(boolean fast)
  ...
  if(Port!=0)
    ...
    for(x=0;x<QueuePosition;x++)
      ...
      if(Queue[x].Type==ESP_TYPE_COMMAND)                                      // Er zijn enkele commando's die geen reacte teruggeven. Zet in deze gevallen de fast mode aan.
        {
...
void QueueReceive(NodoEventStruct *Event)
  ...
  Transmission_ESPOnly=true;                                                   // Uitsluitend Nodo events. Andere signalen worden tijdelijk volledig genegeerd.
  Wait(5, false, 0, true);
  Transmission_ESPOnly=false;
  ...
  TempEvent.Type                = ESP_TYPE_SYSTEM; 
  TempEvent.Command             = SYSTEM_COMMAND_CONFIRMED;      
